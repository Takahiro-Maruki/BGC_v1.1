...
Program BGC_v1.1.cpp to estimate the genotype of each 
individual form pro files of individual high-throughput 
sequencing data of multiple diploid individuals from a 
population, incorporating pre-determined genotype-frequency
estimates by the ML genotype-frequency estimator. In this version, 
genotype probabilities for each individual can be shown.  In this
version 1.1, the output at all sites corresponding to the input
at all sites can be printed out by setting the `-as' option at
one. 
...
int main(int argc, char *argv[]) {
	...
	const char* in_file_name = {"In_BGC.txt"};
	const char* out_file_name = {"Out_BGC.txt"};
	...
	fprintf(outstream, "scaffold\tposition\tbest_p\tbest_error\tpop_coverage\t");
	// printf("scaffold\tposition\tbest_p\tbest_error\tpop_coverage\t");
	if (gp == 0) {
  	...
  	while ( getline(inputFile, line) ) {
			...
			*/
			if (best_p < 1.0) {
				...
				// printf("Finished calculating the conditional probabilities\n");
  				...
  				if (as == 1 || count_geno[1] < tot_count_geno) {
					fprintf(outstream, "%s\t%d\t%f\t%f\t%d\t", scaffold.c_str(), site, best_p, best_error, pop_coverage);
					// printf("%s\t%d\t%f\t%f\t%d\t", scaffold.c_str(), site, best_p, best_error, pop_coverage);
					if (gp == 0) {
						for(ig=1;ig<nsample;ig++) {
		...
		} else {	// Print out the sites without ML estimates
			fprintf(outstream, "%s\t%d\tNA\tNA\t%d\t", scaffold.c_str(), site, pop_coverage);
			// printf("%s\t%d\tNA\tNA\t%d\t", scaffold.c_str(), site, pop_coverage);
			if (gp == 0) {
				for (ig=1;ig<=nsample;ig++){
					if (ig < nsample) {
						fprintf(outstream, "NA\t");
